...
if __name__ == "__main__":
	...
	print(f"ALL STORIES: {allstories}")
  

	# save_location = join(REPO_DIR, "results", feature, subject)

	save_location = '/content/sample_data/FMRI/BOLD_2'
	print("Saving encoding model & results to:", save_location)
	os.makedirs(save_location, exist_ok=True)

	print("TEST SAVE...")
	array_test_1 = [1,2,3,4,5]
	np.savez("%s/TEST_ONLY" % save_location, array_test_1)
	print("SAVE TEST... moving on...")
	...
	# Response
	bold_all = get_response(allstories, subject)
	np.savez("%s/bold_all_v4" % save_location, bold_all)
