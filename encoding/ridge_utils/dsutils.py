    ...
    for w in ds.data:
        ...
        for i in range(num_lsasms):
            ...
            try:
                ...
                #adding a check 
                check = lsasm[str.encode(w.lower())]
                print(f'CHECK = {check}')
...
def make_semantic_model_v2(ds: DataSequence, lsasms: list, sizes: list):
    """
    ds
        datasequence to operate on
    lsasms
    ...
    print('Now in make_semantic_model_v2...Getting Dense Layer Reps for subject')
    model = INSTRUCTOR('hkunlp/instructor-large')
    instruction = "Generate embeddings that fully represents the meaning of the sentence"
    newdata = []
    num_lsasms = len(lsasms)
    for w in ds.data:
        print(f"Processing this word: {w}")
        v = []
        for i in range(num_lsasms):
            lsasm = lsasms[i]
            size = sizes[i]
            try:
                embedding = model.encode([[instruction,w.lower()]])
                print(f"SHAPE OF EMBEDDING: {embedding.shape}")

                #v = np.concatenate(v, embedding)
                v.append([embedding])

                print(f'Sucessful embedding retrieved for word: {w}')
            except KeyError as e:
                v = np.concatenate((v, np.zeros((size)))) #lsasm.data.shape[0],))
        print(f"Finished processing this word: {w}")
        newdata.append(v)
    return DataSequence(np.array(newdata), ds.split_inds, ds.data_times, ds.tr_times)
