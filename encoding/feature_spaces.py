...
from ridge_utils.interpdata import lanczosinterp2D, lanczosinterp2D_v2
from ridge_utils.SemanticModel import SemanticModel
from ridge_utils.dsutils import make_semantic_model_v2, make_semantic_model, make_word_ds, make_phoneme_ds #added make_semantic_model_v2
...
def downsample_word_vectors(stories, word_vectors, wordseqs):
	...
	for story in stories:
		downsampled_semanticseqs[story] = lanczosinterp2D_v2(
...
def get_eng1000_vectors(allstories):
    ...
    wordseqs = get_story_wordseqs(allstories)
    vectors = {}
    print('getting vectors for subject')
    for story in allstories:
        print(f'making semantic model for following story: {story}')
        sm = make_semantic_model_v2(wordseqs[story], [eng1000], [985])
        vectors[story] = sm.data
    return downsample_word_vectors(allstories, vectors, wordseqs)



def get_denselayer_vectors(allstories):
  """Get denselayer vectors (??? Shape???) for specified stories.
  ...
  print('Getting Dense Layer Reps for subject')
  model = INSTRUCTOR('hkunlp/instructor-large')
  instruction = "Generate embeddings that fully represents the meaning of the sentence"
  wordseqs = get_story_wordseqs(allstories)
  vectors = {}

  for story in allstories:
    print(f'Getting representation for following story: {story}')
    story = wordseqs[story]
    print(f'STORY WORDS: {story.data}')
    print('printing alleged ws')

    for w in story.data:
      print(f"word: {w}")

    for idx,chunk in enumerate(story): 
      vectors[story][idx] = model.encode([[instruction,chunk]])
    print(f'Shape of embedding: {vectors[story].shape}')
...
_FEATURE_CONFIG = {
	...
	"eng1000": get_eng1000_vectors, #changed, just assert feature as eng1000
}
